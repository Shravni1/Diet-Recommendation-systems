...
if page == "Automatic Diet Plan":
    ...
    class Display:
        ...
        def display_meal_choices(self, person, recommendations):
            ...
            # Display as a bar chart using Altair
            calories_df = pd.DataFrame({
                "Meal": ["Total Calories"],
                "Calories": [total_calories]
            })
            
            chart = alt.Chart(calories_df).mark_bar(color="#33FF8D").encode(
                x='Meal',
                y='Calories',
                tooltip=['Calories']
            ).properties(
        ...
        if selected_foods:
            ...
            with tab1:
                ...
                pie_data = pd.DataFrame({
                    ...
                    "Grams": [totals['Proteins'], totals['Carbohydrates'], totals['Fats']]
                })
                ...
                calories_from = pd.DataFrame({
                    "Macronutrient": ["Protein", "Carbs", "Fat"],
                    "Calories": [totals['Proteins'] * 4, totals['Carbohydrates'] * 4, totals['Fats'] * 9]
                })
                
                # Create bar chart with Altair
                bar_chart = alt.Chart(calories_from).mark_bar().encode(
                    x="Macronutrient",
                    y="Calories",
                    color="Macronutrient",
                    tooltip=["Macronutrient", "Calories"]
                ).properties(
                    height=400
                )
